...
namespace ff7r
{
	...
	void InstancingBuffer::Init()
	{
		D3D11_BUFFER_DESC tDesc = {};

		tDesc.ByteWidth = sizeof(InstancingData) * m_iMaxCount;
		...
		m_pCopyShader = (CopyBoneShader*)ResourceMgr::GetInst()->FindRes<ComputeShader>(L"CopyBone_CS").Get();
	}
	void InstancingBuffer::SetData()
	{
		...
		memcpy(tMap.pData, &m_vecData[0], sizeof(InstancingData) * m_vecData.size());
	...
	void InstancingBuffer::AddInstancingBoneMat(StructuredBuffer* _pBuffer)
	{
		++m_iAnimInstCount;
		m_vecBoneMat.push_back(_pBuffer);
	}

	void InstancingBuffer::Resize(UINT _iCount)
	{
