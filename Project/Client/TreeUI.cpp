...
namespace ff7r
{
    ...
    TreeNode::~TreeNode()
    {
        SafeDelVector(m_vecChildNode);
    }

    string TreeNode::GetCategory()
    {
        string _category = string("");
        if (m_ParentNode != nullptr)
        {
            _category = m_ParentNode->GetName();
        }
        return _category;
    }
    ...
    void TreeNode::RenderUpdate()
    {
        ...
        if (ImGui::TreeNodeEx(strFinalName.c_str(), flag))
        {
            ...
            else if (ImGui::IsItemHovered() && ImGui::IsMouseDown(ImGuiMouseButton_::ImGuiMouseButton_Right))
            {
                m_Owner->m_RbtDownNode = this;
            }
            else if (ImGui::IsItemHovered() && (ImGui::IsMouseReleased(ImGuiMouseButton_::ImGuiMouseButton_Left) || ImGui::IsMouseReleased(ImGuiMouseButton_::ImGuiMouseButton_Right)))
            {
    ...
    int TreeUI::RenderUpdate()
    {
        if (nullptr != m_RootNode)
        {
            if (m_bShowRoot)
            {
                m_RootNode->RenderUpdate();
            }
            else
            {
                for (size_t i = 0; i < m_RootNode->m_vecChildNode.size(); ++i)
                {
                    m_RootNode->m_vecChildNode[i]->RenderUpdate();
                }
            }
        }
    ...
    void TreeUI::SetSelectedNode(TreeNode* _Node)
    {
        ...
        if (m_LbtDownNode == nullptr && m_RbtDownNode == nullptr)
        {
            return;
        }
        else if (m_LbtDownNode != _Node && m_RbtDownNode != _Node)
        {
        ...
        if (m_SelectedNode)
        {
            ...
            if (select_inst && m_SelectFunc)
            {
                (select_inst->*m_SelectFunc)((DWORD_PTR)m_SelectedNode);
            }
        }
    }
